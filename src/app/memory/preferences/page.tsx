...
export default function PreferencesMemoryPage() {
  ...
  return (
    <WorkspaceShell active="/memory">
      <section className="panel text-zinc-100">
        <p className="text-sm text-zinc-500">记忆中心 / 偏好记忆</p>
        <h1 className="mt-2 text-3xl font-semibold text-zinc-50">偏好记忆</h1>
        <p className="mt-3 max-w-3xl text-sm leading-7 text-zinc-400">这里记录你喜欢的内容、风格和设置，让系统在后续使用中更符合你的习惯。</p>
      </section>

      <section className="panel text-zinc-100">
        <div className="flex gap-2">
          <input value={draft} onChange={(e) => setDraft(e.target.value)} placeholder="记录一个偏好" className="flex-1 rounded-full border border-zinc-800 bg-zinc-800/30 px-4 py-3 text-sm outline-none placeholder:text-zinc-500 focus:border-blue-500/40" />
